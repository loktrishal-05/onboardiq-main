...
export default function SignupPage() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '',
    role: '', department: 'Engineering', seniority: 'L3',
    location: 'Remote', startDate: new Date().toISOString().split('T')[0],
  });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden"
      ...
      <button type="button" onClick={() => step === 2 ? setStep(1) : navigate('/')}
        ...
        <ArrowLeft size={16} />{step === 2 ? 'Back' : 'Home'}
      </button>
        ...
        <div className="glass-card rounded-2xl p-8">
          ...
          <form onSubmit={step === 1 ? (e) => { e.preventDefault(); handleNext(); } : handleSubmit} className="space-y-4">
            {step === 1 ? (
              <>
                ...
                <button type="submit"
                  className="w-full py-3 bg-gold hover:bg-gold/90 text-navy font-semibold rounded-xl transition-all hover:shadow-gold-glow active:scale-[0.99] text-sm">
                  Continue →
                </button>
              </>
            ) : (
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            ...
            <button type="button" onClick={() => navigate('/login')}
              className="text-brand-light hover:text-brand transition-colors font-medium">Sign in</button>
          </p>
        </div>
      </div>
    </div>
